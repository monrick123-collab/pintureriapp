...
### Barter/Trueque — Revisado y corregido (`08b4159`)
4 fixes aplicados: rollback de `approveBarterTransfer` envuelto en try/catch, toast.error en `handleRejectBarter`, limpieza de `selectedBarter`/`barterWithInventory` al cerrar modales, reset completo de form state al cambiar tab. 9 estados reales (no 8): `pending_offer`, `pending_selection`, `counter_proposed`, `pending_approval`, `approved`, `in_transit`, `completed`, `rejected`, `cancelled`. **Lo que NO tocar:** RPCs `process_barter_transfer_bidirectional` y `accept_barter_counter_offer` (atómicos, con FOR UPDATE), notificaciones (8/8 en try/catch), mapeo camelCase/snake_case (completo y correcto), RLS en las 6 tablas barter (todas con anon). **Pendiente que requiere cambio de DB:** `confirmBarterReception` setea `received_by` antes del RPC (si falla, queda inconsistente — requiere modificar el RPC para aceptar el parámetro).

### MunicipalPOS — Resuelto (`29633ed`) — RPC atómico implementado
UI state fixes aplicados: branch change reset completo, tab change cierra modales, post-venta resetea paymentType/selectedClient, finally en handleSetLimit. **Inventario resuelto:** `createMunicipalSale()` ahora usa RPC `process_municipal_sale` atómico que inserta venta + items + descuenta stock en una transacción. Patrón de inventario idéntico a `process_sale` (UPDATE WHERE stock >= quantity). Crédito hace UPSERT en `municipal_accounts`. Precio validado server-side con `extra_percentage` de `clients`. **Lo que NO tocar:** RPCs `add_municipal_payment`/`cancel_municipal_sale`/`edit_municipal_sale` (atómicos, correctos), notificaciones (try/catch en TS, fuera del RPC), mapeo camelCase/snake_case (completo).
