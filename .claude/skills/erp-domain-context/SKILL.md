...
### MunicipalPOS — UI corregida, CRÍTICO pendiente en inventario
UI state fixes aplicados: branch change reset completo, tab change cierra modales, post-venta resetea paymentType/selectedClient, finally en handleSetLimit. **CRÍTICO NO RESUELTO:** `createMunicipalSale()` NO descuenta inventario. `cancel_municipal_sale` SÍ revierte stock. Requiere RPC `process_municipal_sale` atómico (migración SQL). **Lo que NO tocar:** validación server-side de extraPercentage (re-obtiene desde DB), RPCs `add_municipal_payment`/`cancel_municipal_sale`/`edit_municipal_sale` (atómicos, correctos), notificaciones (try/catch), mapeo camelCase/snake_case (completo).
