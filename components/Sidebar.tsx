...
const Sidebar: React.FC<SidebarProps> = ({ user, onLogout }) => {
  ...
  const navItems = [
    ...
    ...(isAdmin ? [
      ...
      { label: 'Finanzas', path: '/finance', icon: 'payments' },
      { label: 'Usuarios y Roles', path: '/users', icon: 'manage_accounts' },
    ] : []),
  ];

  return (
    <aside className="w-64 bg-white dark:bg-[#0f172a] border-r border-slate-200 dark:border-slate-800 flex flex-col flex-shrink-0 h-full transition-all duration-300 z-20 print:hidden">
