...
const POS: React.FC<POSProps> = ({ user, onLogout }) => {
  ...
  const handleRequestDiscount = async () => {
    ...
    if (isAdmin) {
      if (paymentMethod === 'card') {
        toast.warning("Método no permitido", "Los descuentos no aplican en pagos con tarjeta.");
        return;
      }
  ...
  const effectiveDiscount = (paymentMethod === 'cash' || paymentMethod === 'transfer') ? appliedDiscount : null;
  const discountAmount = effectiveDiscount
    ? (effectiveDiscount.type === 'percentage' ? (subtotal * (effectiveDiscount.amount / 100)) : effectiveDiscount.amount)
    : 0;
            ...
            <div className="flex-1 flex overflow-hidden bg-slate-50 dark:bg-slate-900">
                ...
                <div className="p-6 pb-2 space-y-4">
                  {isAdmin && (
                    <select
                      ...
                      onChange={e => { setAdminPosBranchId(e.target.value); setCart([]); setSearch(''); setAppliedDiscount(null); setActiveDiscountRequest(null); setPendingDiscountRequestId(null); }}
              ...
              <div className={`
                ...
                <div className="p-4 border-b dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
                  ...
                  <button onClick={() => { setCart([]); setAppliedDiscount(null); setActiveDiscountRequest(null); setPendingDiscountRequestId(null); }} className="text-slate-400 hover:text-red-500 transition-colors"><span className="material-symbols-outlined text-xl">delete</span></button>
                </div>
                        ...
                        {(['cash', 'card', 'transfer'] as PaymentMethod[]).map(method => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (method === 'card' && appliedDiscount) {
                                toast.warning("Descuento removido", "Los descuentos solo aplican en efectivo o transferencia.");
                                setAppliedDiscount(null);
                                setActiveDiscountRequest(null);
                                setPendingDiscountRequestId(null);
                              }
